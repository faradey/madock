...
import (
	...
	"github.com/faradey/madock/v4/src/model/versions/woocommerce"
...
func Execute(projectName string, projectConf map[string]string, continueSetup bool, args *arg_struct.ControllerGeneralSetup) {
	toolsDefVersions := woocommerce.GetVersions("")
	...
	if args.Download && continueSetup {
		if platformVersion == "" {
			platformVersion = tools.RequireAnswer("Specify WordPress version", "--platform-version")
		}
		if platformVersion != "" {
			toolsDefVersions = woocommerce.GetVersions(platformVersion)
		} else {
	...
	if continueSetup {
		...
		fmt.Println("")
		fmtc.Title("Your WordPress version is " + toolsDefVersions.PlatformVersion)
	...
	if args.Download {
		DownloadWordPress(projectName, platformVersion)
	}

	if args.Install {
		install.WooCommerce(projectName, toolsDefVersions.PlatformVersion, args.SampleData)
	}
}

func DownloadWordPress(projectName, version string) {
	...
	command := []string{
		...
		"cd " + workdir + " " +
			...
			"&& tar xzf wordpress-" + version + ".tar.gz --strip-components=1 " +
