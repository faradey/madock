...
import (
	...
	"github.com/faradey/madock/v4/src/helper/preset"
	"github.com/faradey/madock/v4/src/helper/setup/tools"
	"github.com/faradey/madock/v4/src/model/versions/bigcommerce"
...
func Execute(projectName string, projectConf map[string]string, continueSetup bool, args *arg_struct.ControllerGeneralSetup) {
	...
	if !usePreset && args.PlatformVersion == "" && continueSetup {
		if selected := preset.Choose(preset.GetBigcommercePresets(), "Choose a BigCommerce SDK preset:"); selected != nil {
			toolsDefVersions = selected.Versions
			usePreset = true
		}
	}
