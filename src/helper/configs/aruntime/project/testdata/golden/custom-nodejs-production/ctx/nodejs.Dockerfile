...
# libasound2t64 in Debian trixie and Ubuntu 24.04, so a list pinned today
# breaks the build the day the base image is bumped.
#
# This runs at build time, where we are root. `playwright install --with-deps`
# at runtime cannot work: madock execs as a non-root user, so Playwright
