...
# madock smart entrypoint (Spree Next.js storefront variant):
#   - waits for package.json (project clone happens AFTER container start)
#   - waits for install completion marker
#   - sources .env / .env.local right before exec so SPREE_API_URL +
#     SPREE_PUBLISHABLE_KEY are in process env when next dev boots
#   - Next.js dev binds 3001 by default in spree/storefront's package.json
...
if ! deps_installed; then
  ...
  echo "[madock] Run \"madock install\" to bootstrap the storefront."
