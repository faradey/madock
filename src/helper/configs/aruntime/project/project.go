...
func MakeConf(projectName string) {
	...
	// Last, so an extension writes into a finished directory and nothing here
	// writes over it afterwards. See ctxfiles.go.
	writeCtxFiles(projectName, paths.MakeDirsByPath(pp.CtxDir()))
}
