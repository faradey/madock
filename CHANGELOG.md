...
- **A ciphertext was printed as the password.** A project whose secrets madock-pro had enciphered, read by madock — which has no key — showed `password: ENC:Rv8N5Zv…` in green, as the thing to paste into a client; under pro with a key that did not fit it read `set (44)`, the ciphertext's length. `info` and `db:info` now say `encrypted; cannot be read on this machine`, and madock warns once per run, on stderr, that the project was configured by the other edition and how to store the value in plain text. The existing warning about a key that does not fit moved to stderr as well: both are raised while the config is read, before a command that may be answering in JSON has printed anything
...
Fixed:
- **The log directives were in the vhost, and a project may replace the vhost.** They now live in `ctx/madock-logs.conf`, mounted into `conf.d`, which nginx includes inside the http block — so they cover every server block a project has, including one it ships itself in `.madock/docker/`. Measured on a Magento project that carries its own `nginx/conf/default.conf`: the volume was mounted, the database wrote its slow log, and nginx wrote nothing at all. A server block that sets its own `access_log` still wins for that server, which is correct
...
Added:
- **`php-ldap` in the PHP image.** No madock image carried `ext-ldap` — measured across the whole `docker/` tree on 2026-09-09 — so an application that authenticates against a directory had nowhere to be tested at all. It is installed beside `opcache` and `xmlrpc` rather than in the main list, so a PHP version whose package is missing cannot abort the whole image; ondrej publishes it for 8.3, 8.4 and 8.5 on both amd64 and arm64, checked against the PPA index rather than assumed
- **Logs that a rebuild cannot take away.** Docker keeps a container's stdout inside the container's own directory, so `rebuild` deletes it. Measured on a production machine during an intrusion review on 2026-09-09: three deploys recreated the containers, and the HTTP records for the minute being investigated were gone from disk and from `madock logs`. nginx and the database now write files into a named volume, mounted as `/var/log/madock`, which survives `rebuild` exactly as the database's own volume does and goes away with `project:remove`
...
- A named volume and not a directory under `aruntime`, the second answer to the same question: a bind mount put root-owned files into the project's runtime directory, and `project:remove` runs as the person rather than as root — so the project could not be deleted at all. CI found that as a database refusing to start, because the leftover volume of the undeleted project still carried the old root password
- `log_error` is deliberately left on stderr. MariaDB writes it to a file or to the stream, never both, and the stream is what `madock logs -s db` reads — taking it away to gain a file of startup notes is a bad trade, and a test now asserts the setting is absent
...
Added:
- **The search engine's memory is a setting** — `search/opensearch/memory_limit`, `search/opensearch/heap`, and the same pair for `elasticsearch`, each defaulting to the number already in force (`2512m` for both, `1g` of heap for OpenSearch and `800m` for Elasticsearch — see below for why those differ), so a machine that sets nothing runs what it was running. Measured on a live store: java held 1650 MB of RSS against 12.4 MB of indices and 21 products, on a 5.8 GB machine, and every project paid the same regardless of catalogue size
...
Fixed:
- **OpenSearch never read the heap the snippet set.** The variable was `ES_JAVA_OPTS`, which is Elasticsearch's name; OpenSearch reads `OPENSEARCH_JAVA_OPTS` and ignored it, so every OpenSearch container has run the image's own `-Xms1g -Xmx1g` while the compose file said 800m. Asked of a running node rather than assumed: `heap_max_in_bytes` 1073741824 against a snippet claiming 800m. The `elasticsearch` snippet is correct as it stands — that engine's variable really is `ES_JAVA_OPTS`
...
Added:
- **A host can name the website code it is served as — `nginx/hosts/<key>/code`.** The key has always been the code, and on Magento that is not a naming convention: it is handed to the application as `MAGE_RUN_CODE` through the vhost's `map $http_host $MAGE_RUN_CODE`. So a host added under `www` asked Magento for a website called "www" and got `NoSuchEntityException` — measured on a live store, where every request to the www host answered 500 from 2026-09-06
...
Fixed:
- **The cron check cried wolf on every healthy deploy.** `cron:remove && cron:install && cron:run` exits 1 in the ordinary case on every deploy after the first: `cron:remove` leaves the last block in the crontab whatever it reports, so `cron:install` finds one already there, prints "Crontab has already been generated and saved" and stops the chain. That exit code was read as failure, and "Magento cron setup failed — scheduled jobs may NOT run" went out on deploys where cron was fine. Measured on a live store, release 174: the crontab held exactly one `cron:run` naming that release, `cron` was running by name, a job had completed successfully 23 seconds earlier, and the alarm printed four times in one day
